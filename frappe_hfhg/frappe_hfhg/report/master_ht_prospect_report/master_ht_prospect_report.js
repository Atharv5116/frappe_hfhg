...
frappe.query_reports["Master HT Prospect Report"] = {
  ...
  const updateTotalAmount = function (report) {
	frappe.call({
	  method:
		"frappe_hfhg.frappe_hfhg.report.master_ht_prospect_report.master_ht_prospect_report.execute",
	  args: {
	  ...
	  callback: function (r) {
		if (r.message) {
		  ...
		  let ht_prospect_count = r.message[2].ht_prospect_count;
		  report.page.wrapper.find(".total-amount-header").remove();
  
		  let header_html = `<h5 class="total-amount-header" style="margin-left: 20px; display: flex; align-items: center;">Total Package: ${total_amount}</h5><h5 class="total-amount-header" style="margin-left: 20px; display: flex; align-items: center;">HT Prospect Count: ${ht_prospect_count}</h5>`;
