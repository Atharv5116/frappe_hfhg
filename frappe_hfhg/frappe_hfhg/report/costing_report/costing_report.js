...
frappe.query_reports["Costing Report"] = {
  ...
  filters: [
    ...
    {
      fieldname: "status",
      label: __("Costing Status"),
      fieldtype: "Select",
      options: ["", "Prospect", "Booking", "Refunded"],
    },
    {
...
const updateTotalAmount = function (report) {
  frappe.call({
    method:
      "frappe_hfhg.frappe_hfhg.report.costing_report.costing_report.execute",
    args: {
    ...
    callback: function (r) {
      if (r.message) {
        ...
        let costing_count = r.message[2].costing_count;
        report.page.wrapper.find(".total-amount-header").remove();

        let header_html = `<h5 class="total-amount-header" style="margin-left: 20px; display: flex; align-items: center;">Total Package: ${total_amount}</h5><h5 class="total-amount-header" style="margin-left: 20px; display: flex; align-items: center;">Total Booking Amount: ${total_booking_amount}</h5>
        <h5 class="total-amount-header" style="margin-left: 20px; display: flex; align-items: center;">Costing Count: ${costing_count}</h5>`;
  ...
  $(document).ready(function () {
    ...
    if (breadcrumbContainer.length && !breadcrumbExists) {
      const newBreadcrumb = $(
        '<li><a href="/app/costing/view/list">Costing</a></li>'
      );
